...
print("DEBUG: Starting 9TeamStats.py script.")

# --- Step 1. Retrieve team composition from API ---
print("DEBUG: Retrieving team composition from API endpoint...")
try:
    resp_team = requests.get(TEAM_ENDPOINT, headers=headers)
    resp_team.raise_for_status()
    team_json = resp_team.json()
    print("DEBUG: Team composition API response received:")
    print(json.dumps(team_json, indent=2)[:500] + "\n...")
...
print("DEBUG: Lineup from API:", lineup)
...
for pos in ["1", "2", "3", "4"]:
    ...
    print(f"DEBUG: Position {pos} players: {pos_players}")
...
for bench_key, bench_list in bench.items():
    print(f"DEBUG: Bench group '{bench_key}': {bench_list}")
...
if lineup.get("captain"):
    ...
    print("DEBUG: Added captain:", lineup["captain"])
if lineup.get("vice_captain"):
    team_member_ids.add(lineup["vice_captain"])
    print("DEBUG: Added vice_captain:", lineup["vice_captain"])

print("DEBUG: Team composition from API (before trades):", team_member_ids)

# --- Step 2. Retrieve pending trades from API and update team member IDs ---
print("DEBUG: Retrieving pending trades from API endpoint...")
try:
    resp_trades = requests.get(TRADES_ENDPOINT, headers=headers)
    resp_trades.raise_for_status()
    trades_json = resp_trades.json()
    print("DEBUG: Trades API response received:")
    print(json.dumps(trades_json, indent=2)[:500] + "\n...")
except Exception as e:
...
print("DEBUG: Pending trades result:", trades_result)
...
for trade in trades_result:
    ...
    if old_id in team_member_ids:
        print(f"DEBUG: Trade detected. Replacing {old_id} with {new_id}.")
...
print(f"DEBUG: Connecting to derivative database: {DERIVATIVE_DB}")
try:
    conn = sqlite3.connect(DERIVATIVE_DB)
    cur = conn.cursor()
    print("DEBUG: Connected to 8StatAll.db successfully.")
except Exception as e:
...
print("DEBUG: Executing SQL query:", select_query)
try:
    cur.execute(select_query, team_member_ids)
    team_rows = cur.fetchall()
    print(f"DEBUG: Retrieved {len(team_rows)} rows for team members from player_coach_combined.")
except Exception as e:
    sys.exit(f"ERROR: SQL query execution failed: {e}")

# --- Step 4. Create new database (10TeamStats.db) and copy team records ---
print(f"DEBUG: Creating/connecting to new database: {TEAM_DB}")
try:
...
if team_rows:
    ...
    print("DEBUG: Inserting rows into team_stats using query:", insert_query)
    try:
        cur_team.executemany(insert_query, team_rows)
        conn_team.commit()
        print(f"DEBUG: Inserted {len(team_rows)} rows into team_stats in {TEAM_DB}.")
    except Exception as e:
        sys.exit(f"ERROR: Failed to insert rows into {TEAM_DB}: {e}")
else:
    print("WARNING: No team records found to insert into new database.")

# --- Step 5. Export team records to a CSV file (11TeamStats.csv) ---
print(f"DEBUG: Exporting team records to CSV file: {TEAM_CSV}")
try:
    cur.execute("PRAGMA table_info(player_coach_combined)")
    columns_info = cur.fetchall()
    columns = [info[1] for info in columns_info]
    print("DEBUG: Retrieved column names from 8StatAll.db:", columns)
except Exception as e:
...
try:
    with open(TEAM_CSV, mode="w", newline="", encoding="utf-8") as csvfile:
        ...
        for row in team_rows:
            writer.writerow(row)
    print(f"DEBUG: Exported team records to CSV file: {TEAM_CSV}")
except Exception as e:
...
conn.close()
conn_team.close()
print("DEBUG: Closed all database connections.")
print("DEBUG: 9TeamStats.py completed successfully.")
